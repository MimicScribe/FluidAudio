...
struct OfflineSegmentationProcessor {
    ...
    func process(
        ...
        // binary-argmax masks delete ALL embeddings for sparse speakers (hard ship-gate:
        // 9 merge regressions, sparsest speaker lost in 6/57 files; both changes needed
        // reverting, ratio alone insufficient); transpose fix from the same PR
        // deliberately KEPT; revisit only with a sparse-speaker-preserving alternative
        // (e.g. per-speaker best-N-windows floor instead of a flat ratio).
        ...
        while true {
            ...
            for localIndex in 0..<batchCount {
                ...
                for frameIndex in 0..<frames {
                    ...
                    // ship-gate: 9 merge regressions, sparsest speaker lost in 6/57 files;
                    // both changes needed reverting, ratio alone insufficient); transpose fix
                    // from the same PR deliberately KEPT; revisit only with a
                    // sparse-speaker-preserving alternative (e.g. per-speaker best-N-windows
                    // floor instead of a flat ratio).
